...
def trainval(exp_dict, savedir_base, datadir, reset=False, num_workers=0):
    ...
    for c in range(model.get_state_dict().get('cycle',0), 
                   exp_dict.get('max_cycle', 1)):
        # active learning
        # ----------------
        if 'active_learning' in exp_dict and (s_epoch % exp_dict['max_epoch']) == 0:
            
            print('cycle %d - train set: %d' %(c, len(train_set)))
            s_epoch = 0
            model.waiting = 0
            model.val_score_best = -np.inf
            score_dict = {'cycle':c, 
                            'effort':model.get_effort(), 
                          'n_train':len(train_set), 
                          'n_pool':len(model.get_pool_ind()), 
                          'epoch':0}
            if c > 0:
                score_dict['test_score'] = test_dict['test_score']
            
            score_list += [score_dict]

            # get new train_set
            train_set = model.label_next_batch()

            # reset weights
            if exp_dict['active_learning'].get('reset_weights', False):
                model.init_model()
                model.cuda()
...
if __name__ == "__main__":
    ...
    if args.run_jobs:
        ...
        job_config = {'volume': ['/mnt:/mnt'],
                ...
                'mem': '50',
                'cpu': '8'}
        run_command = ('python trainval.py -ei <exp_id> -sb %s -d %s -nw 2' %  
